...
import global.repository.ApiCallRepository;
import global.repository.ApiKeyRateLimitsRepository;
import global.repository.ApiResponseRepository;
...
public class ApiConnector {
    ...
    public ApiResponse get(ApiCall apiCall) {
        return get(apiCall, null);
    }
    // Second params apiKey for forced apiKey usage
    public ApiResponse get(ApiCall apiCall, ApiKey apiKey) {
        ApiResponse apiResponse = new ApiResponse();

        if (apiCall.getApiEndpoint().isRequiresApiKey() && apiKey == null) {
            apiKey = this.apiKeyManager.getKey(apiCall.getPlatform(), apiCall.getApiEndpoint().getRiotGame());
        }
